"""Shield V1 evidence: orbit views, a real face close-up, action poses, and a
...
a 1.70 m Shield lands on the side of the hood and shows no face at all. A
...
OUT = os.path.join(ROOT, "reports", "shield-v1")
...
# reported success with no shield close-up in the set at all -- the same shape
# of failure as the Warrior's boot shot, which was framed on the knee.
DETAILS = (("hammer", "hand.R", 0.02, 0.46, 0.66),
           ("hammer-grip", "hand.R", -0.02, 0.26, 0.60),
           ("plate", "lower_arm.L", 0.04, 0.78, 0.66),
           ("strap", "lower_arm.L", 0.02, 0.34, 0.60),
           ("helmet", "head", 0.03, 0.44, 0.62),
           ("shoulders", "chest", 0.13, 0.52, 0.66),
...
def main():
    ...
    clear()
    meshes, armature = load(args.shield)
    ...
    for label, azimuth in ORBITS:
        ...
        shoot(os.path.join(OUT, "shield-%s.png" % label))
    ...
    bpy.context.scene.camera = cam
    shoot(os.path.join(OUT, "shield-face.png"), 900, 900)
    ...
    for label, bone_name, lift, span, fov in DETAILS:
        ...
        bpy.context.view_layer.update()
        shoot(os.path.join(OUT, "shield-%s.png" % label), 900, 900)
    ...
    composite(lod_frames, os.path.join(OUT, "shield-lod-comparison.png"))
    ...
    if args.hero:
        ...
        for label, path in (("hero", args.hero), ("warrior", args.warrior),
            ...
            panels.append(frame)
        composite(panels, os.path.join(OUT, "hero-warrior-shield.png"))
        for frame in panels:
            os.remove(frame)

    # The files this run WROTE, not everything sitting in the directory.
    # Listing the directory reported `shield-side.png` and `shield-melee.png`
    ...
    print("SHIELD_EVIDENCE_OK %s" % ", ".join(sorted(WRITTEN)))
