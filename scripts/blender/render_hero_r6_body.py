"""Render R6-A body, boot and helmet silhouette evidence.

This is an evidence-only renderer.  It opens the already-generated Hero blend,
uses the same authored meshes, and does not save or mutate the production
source file.
"""
...
def _prepare(clay=False, silhouette=False):
    ...
    if silhouette:
        material = bpy.data.materials.new("R6_Silhouette")
...
def _render(scene, camera, output_dir, name, location, target=(0, 1.05, 0), lens=58):
...
def main():
    output_dir = os.path.abspath(_arg("--output", os.path.join(os.getcwd(), "reports", "art-previews", "hero-commercial-r6", "R6-A")))
    os.makedirs(output_dir, exist_ok=True)
    views = [
        ("clay-front", (0.0, 5.6, 1.15), (0, 0.0, 1.15), 58, True, False),
        ("clay-side", (5.6, 0.0, 1.15), (0, 0.0, 1.15), 58, True, False),
        ("clay-back", (0.0, -5.6, 1.15), (0, 0.0, 1.15), 58, True, False),
        ("clay-three-quarter", (4.4, 4.4, 1.25), (0, 0.0, 1.10), 58, True, False),
        ("black-silhouette-front", (0.0, 5.6, 1.15), (0, 0.0, 1.15), 58, False, True),
        ("black-silhouette-side", (5.6, 0.0, 1.15), (0, 0.0, 1.15), 58, False, True),
        ("black-silhouette-back", (0.0, -5.6, 1.15), (0, 0.0, 1.15), 58, False, True),
        ("gameplay-distance", (4.8, 6.2, 2.35), (0, 0.0, 1.0), 52, False, False),
    ]
    manifest = {
        "stage": "R6-A",
        "iteration": 1,
        "source": "assets-source/blender/characters/hero.blend",
        "method": "existing authored loop profiles; no new render meshes",
        "views": [],
    }
    ...
    with open(os.path.join(output_dir, "body-manifest.json"), "w", encoding="utf-8") as handle:
        ...
        handle.write("\n")
    print(f"R6-A body renders written to {output_dir}")
