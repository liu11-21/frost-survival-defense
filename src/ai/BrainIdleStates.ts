...
export function tickFormation(brain: FriendlyBrain, dt: number): void {
  ...
  if (b.retargetReady) {
    b.setRetarget(RETARGET_INTERVAL.idle);
    const found = b.isHealer || b.isEngineer || b.isSupporter ? null : b.unit.findHostileTarget();
    if (found) {
      b.setTarget(found);
...
export function tickHold(brain: FriendlyBrain, dt: number): void {
  ...
  if (b.retargetReady) {
    b.setRetarget(RETARGET_INTERVAL.idle);
    b.go(
      b.isHealer ? "acquireHealTarget" : b.isEngineer ? "acquireRepairTarget" : b.isSupporter ? "followFormation" : "acquireTarget",
      "holdRetry",
    );
  }
}
