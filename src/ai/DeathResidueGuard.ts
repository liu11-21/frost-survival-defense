 ...
 * (which only ever looks at *living* allies): every 0.5s, sweep both `world`
 * arrays for anything `!alive` that has outlived `CORPSE_TIME`/`DEATH_HARD_LIMIT`
 * (0.75s / 1s — see `UnitDeath.ts`) by a safe margin and is *still* sitting in
 ...
 */
const RESIDUE_SWEEP_INTERVAL = 0.5;
/** Past `DEATH_HARD_LIMIT` (1s) — never the routine cleanup path. */
const RESIDUE_AGE_THRESHOLD = 1.25;
