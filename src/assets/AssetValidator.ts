...
export function validateAssetContainer(spec: AssetSpec, container: AssetContainer): AssetValidationReport {
  ...
  const lodMeshes = container.meshes.filter((mesh) => {
    const segments = mesh.name.split(":");
    const name = segments[segments.length - 1] ?? mesh.name;
    return name.startsWith("LOD1_PROXY") || name.startsWith("LOD2_PROXY") || name.startsWith("LOD1_PROD") || name.startsWith("LOD2_PROD");
  });
  ...
  const hasLod = (level: 1 | 2): boolean => lodMeshes.some((mesh) => {
    const segments = mesh.name.split(":");
    const name = segments[segments.length - 1] ?? mesh.name;
    return name.startsWith(`LOD${level}_PROXY`) || name.startsWith(`LOD${level}_PROD`);
  });
  if (!hasLod(1) || !hasLod(2)) {
    warnings.push("LOD1/LOD2 geometry is missing; distance switching is unavailable.");
  }
  ...
  return {
    key: spec.key,
    status: missingNodes.length || missingAnimations.length || container.meshes.length === 0 || lodMeshes.length < 2 || !hasLod(1) || !hasLod(2) || skeletonCount < (spec.requiredSkeletons ?? 0) ? "invalid" : "loaded",
